...
public class SchoolDocumentPdfWriter {
     ...
     */
    public byte[] write(Tenant tenant, byte[] logo, String title, List<String> paragraphs, String footer) {
        try (PDDocument document = new PDDocument()) {
            ...
            try (PDPageContentStream content = new PDPageContentStream(document, page)) {
                float y = SchoolBrandedPdf.drawHeader(
                        document, content, tenant, logo, title, page.getMediaBox().getWidth(), page.getMediaBox().getHeight());
